...
class Daily_Insight_Custom_Category_Widget extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array(
			...
			'description'                 => esc_html__( 'Shows Your site&#8217;s categories.','daily-insight' ),
			'customize_selective_refresh' => true,
		);
		parent::__construct( 'tp-widget-custom-category', esc_html__( 'TP: Custom Category Widget', 'daily-insight' ), $widget_ops );
	...
	public function form( $instance ) {
		...
		?>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title:', 'daily-insight' ); ?></label>
			<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		</p>

		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'no_of_category' ) ); ?>"><?php esc_html_e( 'Number of Category Visible:', 'daily-insight' ); ?></label>
			...
			<br><span><b><?php  esc_html_e( 'Enter 0 to show all category', 'daily-insight'); ?></b> </span>
		</p>

		<p><?php esc_html_e('Order:', 'daily-insight'); ?>

			<label for="<?php echo esc_attr( $this->get_field_id( 'order' ) ); ?>"><?php esc_html_e( 'Ascending:', 'daily-insight' ); ?></label>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'order' ) ); ?>"><?php esc_html_e( 'Descending:', 'daily-insight' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'order_by' ) ); ?>"><?php esc_html_e( 'Order By:', 'daily-insight' ); ?></label>
			<select id="<?php echo esc_attr( $this->get_field_id( 'order_by' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'order_by' ) ); ?>" class="widefat" style="width:100%;">
				<option value="name" <?php if ( 'name' == $order_by ) echo 'selected="selected"'; ?>><?php esc_html_e( 'Category Name', 'daily-insight' ); ?></option>
				<option value="count" <?php if ( 'count' == $order_by ) echo 'selected="selected"'; ?>><?php esc_html_e( 'Post Count', 'daily-insight' ); ?></option>
			</select>
		</p>

		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_count') ); ?>"><?php esc_html_e( 'Show Count:', 'daily-insight' ); ?></label>
